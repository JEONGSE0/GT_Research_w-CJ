...
class EmgBleNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('publish_per_channel', True)      
        ...
        qdepth             = self.get_parameter('publish_queue').get_parameter_value().integer_value
        ...
        self.per_ch_decim  = max(1, self.get_parameter('per_channel_decimation').get_parameter_value().integer_value)
        ...
        if self.publish_per_channel:
            for i in range(NUM_CHANNELS):
                # data1..data8 (1-based)
                ch_topic = f"{self.per_ch_prefix}{i+1}"
        ...
        self._sample_count = 0  # decimation 카운터
